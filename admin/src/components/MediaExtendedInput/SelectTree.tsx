...
export const SelectTree = ({
  ...
  const [openValues, setOpenValues] = React.useState(
    getOpenValues(flatDefaultOptions, controlledValue || defaultValue)
  );

  React.useEffect(() => {
    const visibleOptions = flatDefaultOptions.filter((option) => {
      // Always show root items (Media Library)
      if (option.parent === undefined) {
        return true;
      }
      
      // Show items whose parent is in openValues
      return openValues.includes(option.parent);
    });
    
    setOptions(visibleOptions);
  }, [openValues, flatDefaultOptions]);
  ...
  const valueToUse = controlledValue !== undefined ? controlledValue : defaultValue;
  
  // Convert null to undefined for react-select (it doesn't handle null well)
  const sanitizedValue = React.useMemo(() => {
    if (!valueToUse) return undefined;
    return {
      ...valueToUse,
      value: valueToUse.value === null ? undefined : valueToUse.value,
    };
  }, [valueToUse]);

  // Wrap onChange to handle null/undefined conversion
  const handleChange = (value: any) => {
    if (onChange) {
      // Convert undefined back to null for consistency with our data model
